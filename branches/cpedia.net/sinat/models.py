...
from sinat.utils import parse_datetime, parse_html_value, parse_a_href, \
    ...
    def parse(cls, api, json):
        ...
        for k, v in json.items():
            if k == 'user':
                ...
                sus = Status.parse(api, v)
                setattr(status, 'status', sus)  # DEPRECIATED
            elif k == 'created_at':
...
class User_List(Model):
    ...
    def parse(cls, api, json):
        ...
        for k, v in json.items():
            if k=='users':
                setattr(user_list, k, Users.parse_list(api, v))
            else:
...
class Users(Model):
    ...
    def parse(cls, api, json):
        ...
        for k, v in json.items():
            if k=='user':
                setattr(users, k, User.parse(api, v))
            elif k == 'created_at':
                setattr(users, k, parse_datetime(v))
            elif k == 'status':
                setattr(users, k, Status.parse(api, v))
            elif k == 'following':
                # twitter sets this to null if it is false
                if v is True:
                    setattr(users, k, True)
                else:
                    setattr(users, k, False)
            else:
                setattr(users, k, v)
        return users
...
class IDModelList(Model):
    ...
    def parse(cls, api, json):
        ...
        for k, v in json.items():
            if k == 'ids':
                setattr(idslist, k, IDModel.parse(api, v))
            else:
                setattr(idslist, k, v)
        return idslist
